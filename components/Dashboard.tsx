...
import { Briefcase, CheckCircle, FolderKanban, Activity, AlertTriangle, FileSignature, Clock } from 'lucide-react';
  ...
  return (
    <div className="space-y-8">
        ...
        <ChartContainer title="Nível de Prioridade das Atividades">
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={priorityData} layout="vertical" margin={{ top: 0, right: 20, left: -20, bottom: 0 }}>
              ...
              <YAxis dataKey="name" type="category" fontSize={10} fontWeight="bold" axisLine={false} tickLine={false} width={80} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                return (
                  <div key={task.id} className="p-4 bg-slate-50 border border-slate-100 rounded-2xl flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-[9px] font-bold text-indigo-500 uppercase">{task.project}</p>
                    </div>
                    <div className={`px-3 py-1 rounded-full text-[9px] font-black uppercase ${daysLeft <= 2 ? 'bg-red-100 text-red-600' : 'bg-amber-100 text-amber-600'}`}>
        ...
        <div className="lg:col-span-5 bg-slate-900 p-8 rounded-[2rem] shadow-2xl border border-white/10">
...
const AlertItem = ({ value, label, icon, color, onClick }: { value: number; label: string; icon: React.ReactNode; color: string; onClick?: () => void }) => (
  <button
    onClick={onClick}
    disabled={value === 0}
    className="w-full text-left bg-white/5 border border-white/10 p-4 rounded-2xl flex items-center justify-between transition-all hover:bg-white/10 disabled:opacity-50 disabled:pointer-events-none"
  >
    <div className="flex items-center gap-4">
      <div className={`shrink-0 ${color}`}>{icon}</div>
      <p className="text-xs font-bold text-white">{label}</p>
    </div>
    <div className={`px-4 py-1 rounded-full text-sm font-black ${value > 0 ? `${color} bg-white/10` : 'text-slate-500 bg-transparent'}`}>
      {value}
    </div>
  </button>
);
