...
const PlanManagerModal: React.FC<PlanManagerModalProps> = ({ isOpen, onClose, plans, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[70] flex items-center justify-center p-4">
      <div className="bg-white text-slate-800 rounded-[2rem] shadow-2xl w-full max-w-4xl h-[80vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200 border border-slate-200">
        <header className="p-8 bg-slate-50 border-b border-slate-200 flex items-center gap-4">
          ...
          <div className="w-1/3 border-r border-slate-200 flex flex-col bg-slate-50">
            <div className="p-4 space-y-2 border-b border-slate-200">
              <input
                ...
                className="w-full px-4 py-3 bg-white border border-slate-300 rounded-xl text-xs font-bold text-slate-900 outline-none focus:ring-2 focus:ring-teal-500"
              />
              <button onClick={handleAddPlan} className="w-full py-3 bg-teal-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-teal-700 transition"><Plus size={16}/>Adicionar Plano</button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
              {localPlans.map(plan => (
                <button
                  ...
                  className={`w-full p-4 rounded-2xl text-left transition flex justify-between items-center group ${selectedPlanId === plan.id ? 'bg-teal-600 text-white' : 'text-slate-800 hover:bg-slate-200'}`}
                >
                  <span className="font-black text-sm uppercase truncate">{plan.name}</span>
                  {plan.id.startsWith('custom_') && 
                    <span onClick={(e) => { e.stopPropagation(); handleDeletePlan(plan.id); }} className={`p-1 opacity-0 group-hover:opacity-100 rounded-full transition ${selectedPlanId === plan.id ? 'text-teal-200 hover:text-white hover:bg-white/20' : 'text-slate-500 hover:text-red-500 hover:bg-red-100'}`}>
                        <Trash2 size={16} />
                    </span>
                  }
                </button>
          ...
          <div className="w-2/3 flex flex-col">
             {selectedPlan ? (
                ...
                <div className="p-4 flex-1 overflow-y-auto custom-scrollbar space-y-4">
                  <h3 className="text-xs font-black uppercase text-slate-500 tracking-widest px-2">Macroatividades de "{selectedPlan.name}"</h3>
                  <div className="space-y-2">
                    {selectedPlan.macroActivities.map((macro, index) => (
                      <div key={index} className="flex items-center gap-2 p-3 bg-slate-50 border border-slate-200 rounded-xl group">
                        <span className="flex-1 text-xs font-bold text-slate-700 uppercase">{macro}</span>
                        <button onClick={() => handleDeleteMacro(index)} className="p-2 text-slate-500 hover:text-red-500 hover:bg-red-100 rounded-full transition opacity-0 group-hover:opacity-100">
                           <Trash2 size={16} />
                        </button>
                      </div>
                    ))}
                ...
                <div className="p-4 border-t border-slate-200 flex gap-2 bg-slate-50">
                  <input
                    ...
                    className="flex-1 px-4 py-3 bg-white border border-slate-300 rounded-xl text-xs font-bold text-slate-900 outline-none focus:ring-2 focus:ring-teal-500"
                  />
                  <button onClick={handleAddMacro} className="px-5 bg-teal-600 text-white rounded-xl text-xs font-bold uppercase flex items-center justify-center gap-2 hover:bg-teal-700 transition"><Plus size={16} />Adicionar</button>
                </div>
                </>
             ) : (
        ...
        <footer className="p-6 bg-slate-50 border-t border-slate-200 flex justify-end">
          <button onClick={handleSaveAndClose} className="px-10 py-4 bg-teal-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl shadow-teal-500/20 hover:bg-teal-700 transition flex items-center gap-2">
