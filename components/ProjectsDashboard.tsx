...
import { FolderKanban, Activity, PauseCircle, CheckCircle, AlertTriangle, ArrowRight } from 'lucide-react';
...
const ProjectsDashboard: React.FC<ProjectsDashboardProps> = ({ projects, tasks, filteredUser, onNavigateToProject }) => {
  ...
  const projectStats = useMemo(() => {
    ...
    const relevantTasks = tasks.filter(t => !t.deleted && projectIds.has(t.id)); // Assuming task.project is projectId
  ...
  const calculateProjectProgress = (project: Project) => {
    const allMicros = project.macroActivities.flatMap(ma => ma.microActivities);
    if (allMicros.length === 0) return 0;
    const completedMicros = allMicros.filter(mi => mi.status === 'Concluída').length;
    return Math.round((completedMicros / allMicros.length) * 100);
  };
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-7 bg-white p-8 rounded-[2rem] shadow-sm border border-slate-200">
          <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-6">Status dos Projetos</h3>
          <div className="space-y-5 max-h-[400px] overflow-y-auto custom-scrollbar pr-4">
            {userProjects.length > 0 ? userProjects.map(project => {
              const progress = calculateProjectProgress(project);
              const totalMicros = project.macroActivities.flatMap(m => m.microActivities).length;
              const completedMicros = project.macroActivities.flatMap(m => m.microActivities).filter(mi => mi.status === 'Concluída').length;
              ...
              return (
                <div key={project.id} className="space-y-2">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-[9px] font-bold text-slate-400">{completedMicros} de {totalMicros} microatividades concluídas</span>
        ...
        <div className="lg:col-span-5 bg-red-50 p-8 rounded-[2rem] border border-red-200">
           ...
           <div className="space-y-3 max-h-[400px] overflow-y-auto custom-scrollbar pr-2">
             {overdueMicroActivities.length > 0 ? overdueMicroActivities.map(micro => (
                <button 
                  ...
                  className="w-full text-left p-4 bg-white border border-red-200 rounded-2xl group hover:bg-red-100/50 transition"
                >
                  <div className="flex justify-between items-center">
                    <div>
             ...
             )) : <p className="text-center text-red-800/60 text-xs py-20 italic">Nenhuma microatividade vencida!</p>}
           </div>
        </div>
      </div>
    </div>
