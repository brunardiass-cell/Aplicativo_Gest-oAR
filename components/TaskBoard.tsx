    ...
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {activeTasks.map(task => {
          const canEditOrDelete = currentUserRole === 'Admin' || (currentUserRole === 'Membro' && task.projectLead === currentUserName);
          return (
            <div key={task.id} className="bg-white rounded-[2.5rem] border border-slate-200 p-8 shadow-sm hover:shadow-xl hover:border-brand-primary/20 transition-all group flex flex-col h-full relative overflow-hidden">
              <div className="flex justify-between items-start mb-6">
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => onView(task)} className="p-2 text-slate-400 hover:text-brand-primary hover:bg-brand-primary/10 rounded-xl transition" title="Visualizar"><Eye size={18}/></button>
                  {canEditOrDelete && (
                    <>
