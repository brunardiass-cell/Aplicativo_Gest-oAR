...
const PasswordModal: React.FC<PasswordModalProps> = ({ isOpen, onClose, onConfirm, userName }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
      <form onSubmit={handleConfirm} className="bg-white text-slate-900 rounded-[2rem] shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-200">
        ...
        <div className="p-8 space-y-4">
          ...
          <input
            ...
            className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-slate-800 text-slate-900 text-lg font-bold text-center tracking-widest"
          />
        </div>
        ...
        <footer className="p-6 bg-slate-50 border-t border-slate-100 flex flex-col gap-3">
          <button
            type="submit"
            className="w-full py-4 bg-slate-900 text-white rounded-2xl shadow-xl hover:bg-black transition font-black uppercase text-xs tracking-widest flex items-center justify-center gap-2"
          >
