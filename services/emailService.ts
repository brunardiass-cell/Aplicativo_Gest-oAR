...
export const sendNewProjectEmail = async (projectName: string, creatorName: string, managerEmail: string) => {
  ...
  let sentReal = false;
  try {
    sentReal = await MicrosoftGraphService.sendEmail(managerEmail, subject, body);
  } catch (error) {
    console.error("Erro ao enviar e-mail real via Microsoft Graph:", error);
  }
  ...
  notification.innerHTML = `
    ...
    <div>
      <p class="text-[10px] font-black uppercase tracking-widest text-teal-400">${sentReal ? 'E-mail Real Enviado' : 'E-mail não Conectado'}</p>
      <p class="text-xs font-bold">${sentReal ? `E-mail enviado para ${managerEmail}` : `Por favor, conecte à Nuvem Microsoft para disparar para ${managerEmail}`}</p>
    </div>
  `;
  document.body.appendChild(notification);
  setTimeout(() => {
